...
> Removed names: old `StreamCompressor`, `StreamCompressOptions { level: CLevel }`, `compressChunk`, `endStream`, `flushStream`, `recommendedInSize/OutSize`, `setParameter` no longer exist — use `StreamingCompressor` / `CStream` and `compressStream` with `EndDirective {cont,flush,end}`.
