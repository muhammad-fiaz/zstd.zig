...
> Removed names: old `StreamDecompressor.init(allocator, opts)`, `StreamDecompressOptions { dict: ?[]const u8 }`, `decompressChunk`, `recommendedDecompressInSize/OutSize` no longer exist — use `StreamingDecompressor` / `DStream` with `decompressStream` / `decompressAll`.
